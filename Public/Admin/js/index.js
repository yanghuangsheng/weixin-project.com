...
layui.config({
    ...
    tab = layui.tab({
        ...
        closeBefore: function (obj) { //tab 关闭之前触发的事件
            ...
            if (obj.title === 'BTable') {
                ...
                return false;
            }else if(obj.confirm === true){
                layer.confirm('未保存的数据可能会丢失哦，确定要关闭' + obj.title + '吗?', { icon: 3, title: '系统提示' }
                , function (index) {
                    //因为confirm是非阻塞的，所以这里关闭当前tab需要调用一下deleteTab方法
                    tab.deleteTab(obj.tabId);
                    layer.close(index);
    ...
    navbar.set({
        ...
        data: navs
         /* cached:true,
        url: '/Public/Admin/datas/nav.json'*/
    });
    //渲染navbar
    navbar.render();
    //监听点击事件
...
function lock($, layer) {
    ...
    layer.open({
        ...
        success: function (layero, lockIndex) {
            ...
            $.getJSON('/index.php?m=admin&c=login&a=out&_format_=json', null, function (res) {
                if (res.state) {
                    layer.msg(res.msg);
                }
            }, 'json');
            ...
            var unlock = function (un, pwd) {
                console.log(un, pwd);
                //这里可以使用ajax方法解锁
                $.post('index.php?m=admin&c=login', { userName: un, password: pwd }, function (res) {
                    //验证成功
                    if (!res.state) {
                        //关闭锁屏层
